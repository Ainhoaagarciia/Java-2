...
public class CursosDAO implements CursosRepository, CursosAdapter{
    @Override
    ...
    @Override
    public Cursos obtenerporId(int id) {
        try {
            ConexionPostGre conex = Singleton.getInstance();
            Connection conn = conex.getConnection();
            Statement st = conn.createStatement();
            ResultSet lstDatos = st.executeQuery("SELECT * FROM CURSOS WHERE id = " + id);

            if (lstDatos.next()) {
                System.out.println("Película encontrada: " + lstDatos.getString("id") + lstDatos.getString("titulo"));
            } else {
    ...
    public void actualizar(Cursos cursos) {
        try {
            ...
            String sql = "UPDATE CURSOS SET titulo = ?, descripcion = ?, genero_id = ? WHERE id = ?";
            PreparedStatement pst = conn.prepareStatement(sql);

            pst.setString(1, cursos.getTitulo());
            pst.setString(2, cursos.getDescripcion());
            pst.setInt(3, cursos.getTipo().getId());
            pst.setInt(4, cursos.getId());

            int filasAfectadas = pst.executeUpdate();

            if (filasAfectadas > 0) {
                System.out.println("Película actualizada con éxito: " + cursos.getTitulo());
            } else {
                System.out.println("No se encontró una película con ID: " + cursos.getId());
            }
    ...
    public ArrayList<Cursos> obtenerPorTipo(String titulo) {
        try {
            ...
            Connection conn = conex.getConnection();
            Statement st = conn.createStatement();
            ResultSet lstDatos = st.executeQuery("SELECT * FROM cursos c INNER JOIN tipos t ON c.id_tipo = t.id WHERE t.titulo = " + titulo);

            if (lstDatos.next()) {
                System.out.println("Curso encontrado: " + lstDatos.getString("titulo"));
            } else {
